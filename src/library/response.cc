 ...
 */
 ...
 namespace Udjat {
	 ...
	 UDJAT_PRIVATE std::shared_ptr<PyObject> Python::factory(Udjat::Response &response) {

		Python::Guard guard;

		auto self = make_handle(PyObject_CallFunction((PyObject*)&response_type, "O", Py_None));
		...
		// Store Response in the object.
		{
			pyValue *native = ((pyValue *) self.get());
			native->handler = &response;
		}
 ...
 UDJAT_PRIVATE PyObject	* response_alloc(PyTypeObject *type, PyObject *args, PyObject *kwds) {
	debug(__FUNCTION__);
 	return type->tp_alloc(type,0);
 }

 UDJAT_PRIVATE void response_dealloc(PyObject * self) {
	debug(__FUNCTION__);
	Py_TYPE(self)->tp_free(self);
 } 

 UDJAT_PRIVATE int response_init(PyObject *self, PyObject *args, PyObject *kwds) {
	debug(__FUNCTION__);
	return 0;

 }

 UDJAT_PRIVATE void response_finalize(PyObject *self) {
 }
